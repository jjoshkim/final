...
export default class App extends React.Component {
  ...
  getIdeas() {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appA3YQm61MuRQ5Oz/grocery%20list?&view=Grid%20view";

    // Needed for Airtable authorization, replace with your own API key
    let requestOptions = {
      headers: new Headers({
        'Authorization': 'Bearer keyVInu6d0UrYZrFq'
      })
    };
  ...
  upvoteIdea(data, secId, rowId, rowMap) {
    ...
    let airtableUrl = "https://api.airtable.com/v0/appA3YQm61MuRQ5Oz/ideas/" + data.id;

    // Needed for Airtable authorization
    let requestOptions = {
      method: 'PATCH',
      headers: new Headers({
        'Authorization': 'Bearer keyVInu6d0UrYZrFq', // replace with your own API key
        'Content-type': 'application/json'
      }),
      body: JSON.stringify({
        fields: {
          votes: data.fields.cart + 1
        }
      })
    };
  ...
  renderRow(data) {
    return (
      <ListItem style={{ paddingLeft: 20, paddingRight: 20 }}>
        ...
        <Right>
          <Text note>{data.fields.cart} in cart</Text>
        </Right>
      </ListItem>
    )
  }
